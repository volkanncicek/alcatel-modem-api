...
Examples:
  ...
  python examples/signal.py -u http://192.168.1.1 --interval 2
    ...
    parser.add_argument(
        "-p", "--password", help="Admin password (optional, for extended info)"
    )
    parser.add_argument(
        "-i",
        "--interval",
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Only show changes, not every check"
    )
            ...
            try:
                ...
                if last_strength is not None:
                    changed = (
                        ...
                        or network_type != last_network_type
                        or (rssi is not None and rssi != last_rssi)
                    )

                # Build status line
                status_line = (
                    f"Signal: {strength}/5 | Network: {network_type} ({network_name})"
                )
                if rssi is not None:
